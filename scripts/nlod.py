...
DISPLAY_COLUMNS = ("symbol", "rvol", "quant_rating", "lod_time", "lod_price")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Monitor stocks.elevated_rvol for symbols making a new low of day "
            "within a configurable lookback window, update stocks.nlod, and "
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Minutes to look back for new low-of-day events. Defaults to 10.",
    )
...
def empty_ohlcv_frame() -> pd.DataFrame:
    return pd.DataFrame(columns=["Symbol", "Timestamp", "Low"])
...
def fetch_ohlcv_rows(
    ...
    return rows.dropna(subset=["Low"]).sort_values(["Symbol", "Timestamp"])
...
def detect_nlod_rows(
    ...
    for symbol, symbol_rows in ohlcv_rows.groupby("Symbol", sort=False):
        ...
        previous_lod = data["Low"].cummin().shift(1)
        new_lod = previous_lod.isna() | (data["Low"] < previous_lod)
        recent_new_lods = data.loc[new_lod & (data["Timestamp"] >= cutoff)]
        if recent_new_lods.empty:
            continue

        latest_lod = recent_new_lods.iloc[-1]
        ...
        output_rows.append(
            {
                ...
                "lod_time": pd.Timestamp(latest_lod["Timestamp"]),
                "lod_price": numeric_or_none(latest_lod["Low"]),
            }
        )
...
def write_nlod_rows(engine: Engine, rows: pd.DataFrame) -> None:
    if rows.empty:
...
def refresh_nlod(
    ...
    write_nlod_rows(stocks_engine, nlod_rows)
    return nlod_rows
...
class NlodGUI:
    def __init__(self, root: tk.Tk, config: MonitorConfig) -> None:
        ...
        self.root.title("New Low of Day")
    ...
    def render_rows(self, rows: list[dict[str, Any]]) -> None:
        ...
        elif self.sort_column == "lod_time":
            sorted_rows.sort(
                key=lambda row: pd.Timestamp(row.get("lod_time", pd.Timestamp.min)),
                reverse=self.sort_descending,
            )
        else:
            sorted_rows.sort(
        ...
        for row in sorted_rows:
            self.tree.insert(
                ...
                values=(
                    ...
                    format_time(row.get("lod_time")),
                    format_number(row.get("lod_price")),
                ),
            )
        ...
        try:
            while True:
                ...
                if message_type == "status":
                    ...
                    messagebox.showerror("NLOD Failed", str(payload))
        except queue.Empty:
            pass
    ...
    def worker_loop(self) -> None:
        try:
            ...
            self.output_queue.put(("status", "Monitoring new lows of day..."))
            ...
            while not self.stop_event.is_set():
                ...
                rows = refresh_nlod(stocks_engine, stream_engine, active_config)
                ...
                self.output_queue.put(
                    (
                        ...
                        f"nlod={len(records)} lookback={active_config.lookback_minutes}m",
                    )
                )
